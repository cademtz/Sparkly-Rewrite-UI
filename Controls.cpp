...
void InitTextures()
{
	...
	for (int h = 0; h < 128; h++)
	{
		for (int w = 0; w < 128; w++)
		{
			Color hsv = Color::FromHSB(float(w) / 128, float(128 - h) / 128.f, 1);
...
int Groupbox::Draw(bool mouseOver)
{
	...
	gDraw.DrawLine(x, y, x, y + h, Color(80));
	gDraw.DrawLine(x + w, y, x + w, y + h, Color(80));
	gDraw.DrawLine(x, y + h, x + w, y + h, Color(80));

	gDraw.DrawString(x + (w / 2) - (strw / 2), y - (strh / 2), Color(110), name, gFonts.verdana_bold);
	gDraw.DrawLine(x, y, x + (w / 2) - (strw / 2) - 5, y, Color(80));
	gDraw.DrawLine(x + (w / 2) + (strw / 2) + 5, y, x + w, y, Color(80));
...
{
	if (value)
		gDraw.DrawRect(x + 2, y + 2, h - 4, h - 4, Color(35, 135, 230));

	gDraw.OutlineRect(x, y, h, h, Color(100));
...
int Slider::Draw(bool mouseOver)
{
	...
	if (mouseOver)
	{
		...
		gDraw.DrawRect(x, y + 17, percent, 4, Color(85, 85, 95));
	gDraw.DrawString(x + nw + 3, y + 9, Color(125, 125, 130), to_string(value), gFonts.verdana_bold);
...
void DrawListbox(void* data, size_t Index)
{
	...
	for (size_t i = 0; i < listbox->list.size(); i++)
	{
		if (gMenu.mouseOver(x, y, w, 15))
		{
			...
			gDraw.DrawRect(x, y, w, 16, Color(44, 44, 55));
		}
...
int ColorPicker::Draw(bool mouseOver)
{
	...
	if (!mouseOver)
		gDraw.DrawRect(x, y + 17, w, 5, Color(0, 150));
...
void DrawColorBox(void* data, size_t Index)
{
	...
	gDraw.OutlineRect(X + 4, Y + 29, colorpicker.GetWidth() + 2, colorpicker.GetHeight() + 2, Color(58, 58, 70));
	colorpicker.Draw(X + 5, Y + 30, Color(255 - (float(vcursory) / brightness.GetHeight()) * 255));

	gDraw.OutlineRect(X + colorpicker.GetWidth() + 9, Y + 29, 16 + 2, brightness.GetHeight() + 2, Color(58, 58, 70));
	brightness.Draw(X + colorpicker.GetWidth() + 10, Y + 30);

	gDraw.OutlineRect(X + 4, Y + 4, colorpicker.GetWidth() + 23, 22, Color(58, 58, 70));
...
int KeyBind::Draw(bool mouseOver)
{
	...
	gDraw.DrawRect(x, y + TEXTH, w, TEXTH, Color(42, 42, 48));
	gDraw.OutlineRect(x, y + TEXTH, w, TEXTH, Color(60));

	gDraw.DrawString(x + 3, y + TEXTH, mouseOver ? clr : Color(125, 125, 130), gKey.KeyToString(key), gFonts.verdana_bold);

	gDraw.DrawRect(x + w - 10, y + TEXTH + 5, 6, 1, Color(125, 125, 140));
	gDraw.DrawRect(x + w - 10, y + TEXTH + 7, 6, 1, Color(125, 125, 140));
	gDraw.DrawRect(x + w - 10, y + TEXTH + 9, 6, 1, Color(125, 125, 140));
...
void DrawKeyBindBox(void* data, size_t Index)
{
	...
	gDraw.DrawString(x + 5, y + 5, Color(100), kb->name, gFonts.verdana_bold);
	y += 21, h -= 21;
	...
	gDraw.OutlineRect(x + 6, y + 6, w - 12, 18, Color(80));
	gDraw.DrawRect(x + 7, y + 7, w - 14, 16, Color(hover_setkey ? 65 : 40));
	gDraw.DrawString(x + 9, y + 8, Color(200), bWait ? "Select a key (or Escape)" : gKey.KeyToString(kb->key), gFonts.verdana_bold);
